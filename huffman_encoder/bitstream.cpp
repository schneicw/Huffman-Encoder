 ...
 *
 * @author Keith Schwarz, Eric Roberts, Marty Stepp, John Stratton
 * @version 2017/08/29
 * - decoupled from the Standford library environment for independent use
 * @version 2016/11/12
...
void obitstream::writeBit(int bit) {
    assert((bit == 0 || bit == 1) && "Trying to write a non-binary bit");
    assert(is_open());
